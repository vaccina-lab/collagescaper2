/* SALVAGE/9 — mirrors, filters, grading, raster sentry, paced queues.
   Six mirrors with independent rate windows. Hard rules (each caused a real
   outage — don't re-break): throttled no-op ≠ dry well; real empty answer →
   rotate vein (resume mid-page); all-dupes → walk page forward; per-tap
...
/* ================================================================== */
/*  FILTER STACK — order matters (see HANDOFF.md)                      */
/* ================================================================== */
...
const BANNED_HARD = new RegExp(`\\b(${HARD_WORDS.map(w2 => w2.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})\\b`, 'i');
...
/* sport / activity photos — die on EVERY lineage; TECH_ESCAPES protects real
   mechanical vocabulary (patent drawings of "runner gear", "climbing mechanism") */
...
const BANNED_ACTIVITY = new RegExp(`\\b(${ACTIVITY_WORDS.map(w2 => w2.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})\\b`, 'i');
...
const TEXT_WORDS = [
  ...
  /* boring text / news / prose pages */
  'news article', 'press clipping', 'newspaper page', 'newspaper clipping', 'article', 'text page',
  'page of text', 'full page text', 'sermon', 'thesis', 'dissertation', 'essay', 'prose', 'verse text',
  'library book', 'open book text', 'printed page', 'page scan', 'text scan',
];
const BANNED_TEXT_WORDS = new RegExp(`\\b(${TEXT_WORDS.map(w2 => w2.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})\\b`, 'i');
...
/* STRONG_ART = printmaking techniques that can NEVER be a photograph — an
   unconditional pass for person subjects. WEAK_ART = media words that are
   usually drawn/painted but could be photographic, so they only pass when no
   PHOTO_WORDS appear anywhere. PHOTO_WORDS are the tell that flips a "person"
   title from "artwork of a person" to "photograph of a person". */
const STRONG_ART = /\b(engrav|etching|woodcut|wood-cut|wood cut|lithograph|intaglio|linocut|linoleum|woodblock|wood block|chromolithograph|mezzotint|aquatint|drypoint)\b/i;
const WEAK_ART = /\b(plate|print|drawing|illustrat|poster|sketch|painting|artwork|manuscript|illuminated|study|miniature|cameo|statue|sculpture|bust|relief|fresco|icon)\b/i;
const PHOTO_WORDS = /\b(photo|photograph|photographic|photographed|snapshot|camera|dslr|selfie|press photo|news photo|stock photo|gelatin silver|silver print|headshot|polaroid|daguerreotype|tintype)\b/i;
...
function creditBlob(item: RemoteItem): string {
  return `${item.creator ?? ''} ${item.provider ?? ''}`;
}

export function allowedItem(item: RemoteItem, family: Family): boolean {
  const t = item.title;
  const blob = creditBlob(item);
  ...
  if (BANNED_HUMAN.test(t)) {
    /* a printmaking technique in the title can never be a photo → keep */
    if (STRONG_ART.test(t)) return true;
    const hasPhoto = PHOTO_WORDS.test(t) || PHOTO_WORDS.test(blob);
    if (!hasPhoto) {
      /* drawn / painted / illustrated, or attributed to an artist → keep */
      if (WEAK_ART.test(t)) return true;
      if (ARTIST_PROOF.test(blob)) return true;
    }
    /* photographic or unproven person subject → bin. Institutional credit
       alone is deliberately NOT an escape: museums digitize millions of
       real-person photographs. */
    return false;
  }
  return true;
}
...
export function analyzePlate(img: HTMLImageElement): PlateScan {
  ...
  /* text page = light paper + many rows of short horizontal runs + low chroma.
     Engravings are hatched (not short horizontal runs) so they don't trip this. */
...
export function photoJunk(
  ...
  /* visible skin on a color photo = flesh & blood. Engravings / charts are
     low-chroma so they never reach here. */
  if (scan.skin >= 0.03) return 'flesh & blood (skin + photo signature)';
  /* a photo-signature image whose title names a person is a real-person
     photo even without visible skin (clothed portraits, group shots). */
