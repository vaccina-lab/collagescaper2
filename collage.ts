...
export function textLayer(text: string, cx: number, cy: number): DeskLayer {
  return { id: `ly-t-${Date.now().toString(36)}`, kind: 'text', name: text.slice(0, 18), x: cx, y: cy, w: 520, aspect: 520 / 120, rot: 0, opacity: 1, blend: 'source-over', text, fontSize: 96 };
}
