...
/* Fire a real download from an object URL. Some browsers (esp. iPad/Safari)
   ignore programmatic clicks on detached anchors, so we attach to the DOM,
   click, and only revoke after a delay. Returns true if it believes it fired. */
function fireDownload(url: string, name: string): boolean {
  try {
...
export default function App() {
  ...
  const downloadOne = useCallback(async (sp: Specimen) => {
    try { await exportSingle(sp); c.say('sys', `exported ${sp.code} → jpg`); }
  ...
  const handleBatch = useCallback(async (mode: 'full' | 'jpg1400') => {
    ...
    try {
      ...
      /* THE FIX: actually trigger the download. Before this, the code only
         armed a "SAVE ARCHIVE" banner and never fired a download, so the
         ZIP sat in memory and nothing ever saved. Now we fire a real
         anchor-click download immediately, and keep the banner armed as a
         manual fallback (for browsers that block programmatic downloads). */
      const fired = fireDownload(res.url, res.name);
      c.say('cut', mode === 'jpg1400'
        ? `zip packed — ${res.count} JPGs @1400px (${res.mb} MB)${mended}${fired ? ' · download fired' : ' · tap SAVE ARCHIVE'}`
        : `zip packed — ${res.count} plates (${res.mb} MB)${mended}${fired ? ' · download fired' : ' · tap SAVE ARCHIVE'}`);
    } catch {
      setBatch(null);
