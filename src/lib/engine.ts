/* SALVAGE/9 — crawler engine hook.
   40 built-in taps across 12 lineages (see HANDOFF.md; never truncate this
   list — a short list is a user-visible regression). Isolation runs in a
   worker (see cutout.ts). Tray caps at 2000 and auto-HOLDs; clear/cull
   resume, single removals don't. */

import { useCallback, useEffect, useRef, useState } from 'react';
import type { Family, Specimen } from './types';
import { gradePlate, mirrorStats, queueFor, analyzePlate, photoJunk, type ProviderPolicy } from './remote';
...
export interface SourceDef {
  ...
  spawnP: number; hue: number; builtin: boolean;
  query?: string; queries?: string[];
  off?: boolean;            /* mounted dark by default */
  policy?: ProviderPolicy;  /* provider allowlist override */
}
...
/* ------------------------------------------------------------------ */
/*  BUILT-IN TAP MANIFEST — 40 taps. DO NOT TRUNCATE.                  */
/* ------------------------------------------------------------------ */
export const BUILTIN_DEFS: SourceDef[] = [
  ...
  { id: 'xray', name: 'Bone Yard', code: 'BONY', blurb: 'radiographic plates', family: 'anatomy', spawnP: 0.14, hue: 196, builtin: true, queries: ['vintage x-ray photograph', 'radiograph vintage photograph', 'fluoroscopy photograph', 'vintage medical x-ray'] },
  /* webcore */
  ...
  { id: 'cross', name: 'Crossworks', code: 'CRSX', blurb: 'crosses & cruciforms', family: 'tarot', spawnP: 0.14, hue: 334, builtin: true, queries: ['ornate cross engraving', 'celtic cross illustration', 'medieval cross engraving', 'crucifix illustration vintage', 'iron cross engraving', 'religious cross ornament', 'cross pattee heraldry'] },
  /* vhs */
  ...
  /* dore */
  { id: 'doreInferno', name: 'Doré Inferno', code: 'D-INF', blurb: 'the comedy, the circles, the fire', family: 'dore', spawnP: 0.24, hue: 178, builtin: true, queries: ['gustave dore engraving', 'dore divine comedy', 'dore inferno', 'dore don quixote', 'dore raven', 'dore london pilgrimage', 'wood engraving 19th century', 'dore illustration plate'] },
  { id: 'doreLondon', name: 'Doré London', code: 'D-LDN', blurb: 'pilgrimage through the fog', family: 'dore', spawnP: 0.16, hue: 190, builtin: true, queries: ['dore london pilgrimage', 'dore over london rooftops', 'dore street scene engraving', 'victorian london engraving', 'dore urban landscape'] },
  { id: 'burin', name: 'Burin Bureau', code: 'BURIN', blurb: 'copperplate & woodcut', family: 'dore', spawnP: 0.15, hue: 66, builtin: true, queries: ['copperplate engraving', 'vintage etching print', 'wood engraving victorian book illustration', '19th century book illustration engraving', 'engraved plate antique print', 'mezzotint portrait engraving', 'steel engraving scene', 'wood engraving dramatic', 'antique print engraving battle', 'engraving ship storm sea', 'woodcut animal print', 'vintage animal engraving'] },
  { id: 'bestiary', name: 'BESTIARY', code: 'BEST', blurb: 'woodcut beasts, wild & tame', family: 'dore', spawnP: 0.18, hue: 130, builtin: true, policy: 'open', queries: ['woodcut animal', 'vintage animal engraving', 'zoological illustration', 'natural history plate animal', 'woodcut wolf', 'woodcut bear', 'raven woodcut', 'owl woodcut', 'serpent snake engraving', 'boar woodcut', 'tiger engraving print', 'woodcut deer stag', 'hare rabbit engraving', 'fox woodcut', 'lion engraving', 'octopus engraving'] },
  /* geometry */
  ...
  { id: 'cursedFeed', name: 'CURSED FEED', code: 'CURS', blurb: 'cursed images & deep-fried stupidity', family: 'meme', spawnP: 0.17, hue: 58, builtin: true, queries: ['cursed image', 'cursed stock photo', 'bad luck brian', 'deep fried meme', 'surreal meme image', 'absurd meme', 'weird cursed photo', 'unnerving image', 'cursed object photo', 'glitch meme', 'cursed animals photo', 'low quality meme image', 'eerie funny photo', 'shitpost image'] },
  { id: 'signalText', name: 'SIGNAL TEXT', code: 'SIGT', blurb: 'transmission artifacts & dead signals', family: 'meme', spawnP: 0.14, hue: 206, builtin: true, queries: ['signal interference photo', 'broadcast interference', 'television snow texture', 'analog signal decay', 'radio static visual', 'test tone screen', 'video feedback art', 'vhs tracking error', 'magnetic tape damage', 'ghosting signal photo', 'analog glitch texture', 'broadcast ghost image'] },
  { id: 'corpoSludge', name: 'CORPO-SLUDGE', code: 'CRPS', blurb: 'acid-green corporate grotesque', family: 'meme', spawnP: 0.16, hue: 110, builtin: true, queries: ['corporate surrealism', 'grotesque stock photo', 'liminal office space', 'surreal office photo', 'bio-punk art', 'neon green pink aesthetic', 'fisheye lens grotesque', 'body horror illustration', 'weird 90s computer', 'corporate hellscape', 'grotesque neon sculpture', 'surreal cubicle photo', 'HR nightmare surreal', 'corporate Memphis parody'] },
...
/* ------------------------------------------------------------------ */
interface Settings {
  gate: number; autoCut: boolean; autoIso: boolean; cutGate: number;
  showRejects: boolean; pace: Pace; keepAwake: boolean; sourcesOn: Record<string, boolean>;
}
function loadSettings(): Settings {
  const fallback: Settings = { gate: 72, autoCut: true, autoIso: true, cutGate: 55, showRejects: true, pace: 'rapid', keepAwake: false, sourcesOn: {} };
  try {
    const raw = localStorage.getItem('salvage9.settings.v1');
    if (!raw) return fallback;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return fallback;
    return { ...fallback, ...parsed, sourcesOn: parsed.sourcesOn && typeof parsed.sourcesOn === 'object' && !Array.isArray(parsed.sourcesOn) ? parsed.sourcesOn : {} };
...
function loadCustomDefs(): SourceDef[] {
  try {
    ...
    return parsed
      .filter((d): d is Record<string, unknown> => isPlainObject(d) && typeof d.id === 'string')
...
function defsFromStorage(): Record<string, SourceDef> {
  ...
  for (const d of BUILTIN_DEFS) map[d.id] = d;
  for (const d of loadCustomDefs()) map[d.id] = { ...d, builtin: false };
  return map;
}
...
function normalizeTray(list: unknown): Specimen[] {
  ...
  return list
    .filter((s): s is Record<string, unknown> => isPlainObject(s) && typeof s.id === 'string')
    .map(s => ({ ...(s as unknown as Specimen), cutoutSrc: undefined, isoState: undefined }));
}
function loadRawTray(): Specimen[] {
  try {
    const raw = localStorage.getItem('salvage9.tray.v1');
    return normalizeTray(raw ? JSON.parse(raw) : []);
  } catch { return []; }
}
...
export function useCrawler() {
  ...
  const [pace, setPaceState] = useState<Pace>(saved.current.pace === 'cruise' ? 'cruise' : 'rapid');
  ...
  const [sources, setSources] = useState<SourcesState>(() => {
    ...
    for (const d of Object.values(defs0.current)) {
      s[d.id] = { on: savedOn[d.id] ?? !d.off, health: 'ok', until: 0, pulled: 0 };
    }
    return s;
  });

  /* ---- batched state core: refs are truth, UI commits coalesce ---- */
  const [feed, setFeed] = useState<Specimen[]>([]);
  const feedRef = useRef<Specimen[]>(feed);
  const [tray, setTray] = useState<Specimen[]>(() => loadRawTray().slice(0, TRAY_CAP));
  ...
  const trayFlushT = useRef<number | null>(null);
  const trayDirty = () => {
    if (trayFlushT.current !== null) return;
    trayFlushT.current = window.setTimeout(() => {
      trayFlushT.current = null;
      setTray(trayRef.current);
    }, 400);
  };
  const trayCommit = () => {
    if (trayFlushT.current !== null) { window.clearTimeout(trayFlushT.current); trayFlushT.current = null; }
    setTray(trayRef.current);
  };
  ...
  const holdForTray = () => { heldByTray.current = true; setTrayHeld(true); setRunning(false); };
  const releaseTrayHold = (resume: boolean) => {
    if (!heldByTray.current) return;
  ...
  useEffect(() => {
    saveSettings({
      gate, autoCut, autoIso, cutGate, showRejects, pace, keepAwake,
      sourcesOn: Object.fromEntries(Object.entries(sources).map(([k, v]) => [k, v.on])),
    });
  }, [gate, autoCut, autoIso, cutGate, showRejects, pace, keepAwake, sources]);
  useEffect(() => {
  ...
  useEffect(() => {
    try { localStorage.setItem('salvage9.sources.v1', JSON.stringify(Object.values(defs).filter(d => !d.builtin))); } catch { /* quota */ }
  }, [defs]);
  ...
  const isolateBest = async (spec: Specimen): Promise<{ cutoutSrc: string; cutEngine: 'ink' | 'flood' | 'model'; cutScore: number } | null> => {
    const url = spec.thumb ?? spec.dataUri;
    if (!url || !/^https?:/i.test(url)) return null;
    const gateNow = cfg.current.cutGate;
    try {
      const fast = await isolateFromUrl(url, 'fast', 1400);
      const img = await loadImageDataUrl(fast.dataUrl);
      const g = gradeCutout(img);
      if (g.score >= gateNow) return { cutoutSrc: fast.dataUrl, cutEngine: fast.engine, cutScore: g.score };
      try {
        const fine = await isolateFromUrl(url, 'fine', 1400);
        const img2 = await loadImageDataUrl(fine.dataUrl);
        const g2 = gradeCutout(img2);
        if (g2.score >= gateNow || g2.score > g.score) return { cutoutSrc: fine.dataUrl, cutEngine: fine.engine, cutScore: g2.score };
      } catch { /* keep fast result if gate allows */ }
      if (g.score >= gateNow) return { cutoutSrc: fast.dataUrl, cutEngine: fast.engine, cutScore: g.score };
      return null;
    } catch {
      return null;
  ...
  const isoPump = async () => {
    while (isoQueue.current.length > 0) {
      if (isoActive.current >= 2) {
        window.setTimeout(() => void isoPump(), 800);
        return;
      }
      isoActive.current++;
      try {
        ...
        try {
          ...
          if (best) {
            markIso(id, 'done', best.cutoutSrc, best.cutEngine, best.cutScore);
            isoStats.current.cut++;
            if (!isoFirstCut.current) {
              isoFirstCut.current = true;
              say('cut', `✂ first cut landed (${best.cutEngine}, grade ${best.cutScore}) — isolation is live`);
            }
          } else {
    ...
    /* self-reschedule in case new work landed while winding down */
    if (isoQueue.current.length > 0) void isoPump();
  };
    ...
    for (const id of ids) {
      if (isoQueue.current.length >= ISO_MAX_QUEUE) {
        if (!isoOverflowWarned.current) {
          ...
          say('warn', `isolation backlog full at ${ISO_MAX_QUEUE} — stragglers deferred to the sweep`);
        }
        break;
      }
  ...
  /* inline cut-time isolation: runs as the plate is cut into the tray */
  const cutWithIso = (spec: Specimen, mode: 'auto' | 'manual' | 'override') => {
    doCut(spec, mode);
    if (!cfg.current.autoIso || !spec.remote) return;
    if (isoInline.current >= MAX_INLINE_ISO) {
      enqueueIso([spec.id]);
      return;
    }
    isoInline.current++;
    markIso(spec.id, 'queue');
    void (async () => {
      try {
        markIso(spec.id, 'work');
        const best = await isolateBest(spec);
        if (best) {
          markIso(spec.id, 'done', best.cutoutSrc, best.cutEngine, best.cutScore);
          isoStats.current.cut++;
          if (!isoFirstCut.current) {
            isoFirstCut.current = true;
            say('cut', `✂ first cut landed (${best.cutEngine}, grade ${best.cutScore}) — isolation is live`);
  ...
  const doCut = (spec: Specimen, mode: 'auto' | 'manual' | 'override') => {
    ...
    trayRef.current = [...trayRef.current, spec]; /* append at the tail */
    if (mode === 'auto') trayDirty(); else trayCommit();
    say('cut', `✂ ${spec.code} → tray [${mode}] grade ${spec.score}`);
  };

  const cut = (spec: Specimen) => cutWithIso({ ...spec, verdict: 'pass', state: 'judged' }, spec.verdict === 'reject' ? 'override' : 'manual');
  ...
  const judge = (spec: Specimen) => {
    ...
    if (!live) return; /* dropped by sentry / purge */
    ...
    if (verdict === 'pass') {
      ...
      say('pass', `✓ PASS ${score} — ${spec.code} “${spec.archetype}”${cfg.current.autoCut ? ' → auto-cut' : ''}`);
  ...
  const spawn = (def: SourceDef) => {
    ...
    probe.onload = () => {
      ...
      else if (scan.textPage) reason = 'typeset page (wall of text)';
  ...
  /* isolation engine status announcement */
  useEffect(() => {
    const off = onIsoProgress((pct, modelName) => {
      if (pct === null && modelName) say('sys', `isolation engine online: ${modelName}`);
    });
    return off;
