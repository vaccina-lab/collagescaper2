/* SALVAGE/9 — crawler engine hook.
   ...
   never shorten this). Isolation runs in a worker (see cutout.ts). Tray caps
   at 2000 and auto-HOLDs; clear/cull resume, single removals don't. */
...
export const BUILTIN_DEFS: SourceDef[] = [
  ...
  { id: 'print', name: 'Press Room', code: 'PRESS', blurb: 'print machinery & shops', family: 'patent', spawnP: 0.14, hue: 46, builtin: true, queries: ['vintage printing press engraving', 'lithography machine illustration', 'print shop composing room', 'wood type case photograph', 'old print shop interior', 'engraved printing press'] },
  /* ---- anatomy ---- */
  ...
  /* ---- dore ---- */
  { id: 'dore', name: 'Doré Inferno', code: 'DORE-I', blurb: 'the comedy, the circles, the fire', family: 'dore', spawnP: 0.21, hue: 178, builtin: true, queries: ['Doré Divine Comedy', 'Doré Inferno', 'Dante Inferno engraving', 'Gustave Doré hell', 'Doré Purgatorio', 'Doré Paradise Lost', 'Dante circle of hell illustration', 'Gustave Doré Divine Comedy plate', 'Inferno canto engraving', 'Doré Charon', 'fallen angels engraving Doré', 'Gustave Doré demons'] },
  { id: 'dorelondon', name: 'Doré London', code: 'DORE-L', blurb: 'pilgrimage, Quixote & the raven', family: 'dore', spawnP: 0.18, hue: 190, builtin: true, queries: ['Doré London A Pilgrimage', 'Gustave Doré London', 'Doré Don Quixote', 'Doré raven Poe', 'Gustave Doré raven illustration', 'Doré Contes Drolatiques', 'Doré Arabian Nights', 'Gustave Doré Bible', 'Doré Crusoe', 'Doré Atala', 'London pilgrimage engraving', 'Gustave Doré street scene'] },
...
export function useCrawler() {
  ...
  const isolateBest = async (spec: Specimen): Promise<{ cutoutSrc: string; cutEngine: 'ink' | 'flood' | 'model'; cutScore: number } | null> => {
    ...
    try {
      ...
      if (g.score >= gate) return { cutoutSrc: fast.dataUrl, cutEngine: fast.engine, cutScore: g.score };
      /* second attempt at higher fidelity */
      try {
        const fine = await isolateFromUrl(url, 'fine', 1400);
        const img2 = await loadImageDataUrl(fine.dataUrl);
        const g2 = gradeCutout(img2);
        if (g2.score >= gate || g2.score > g.score) return { cutoutSrc: fine.dataUrl, cutEngine: fine.engine, cutScore: g2.score };
  ...
  const setCutGate = (v: number) => {
    ...
    /* retroactive pass: re-run any tray plate that hasn't cleared the new gate */
