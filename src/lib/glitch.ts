...
export function renderGlitch(src: GlitchSource, w: number, h: number, p: GlitchParams): { canvas: HTMLCanvasElement; pipe: Pipeline } {
  ...
  /* DAMAGE: master gain — scales every other channel and adds base corruption */
  if (p.damage > 0) {
    const a = p.damage / 100;
    try {
      const img = ox.getImageData(0, 0, w, h);
      const d = img.data;
      for (let i = 0; i < d.length; i += 4) {
        if (R() < a * 0.06) { const v = (R() - 0.5) * 2 * 120 * a; d[i] += v; d[i + 1] += v * 0.7; d[i + 2] += v; }
      }
      ox.putImageData(img, 0, 0);
    } catch { /* composite fallback: skip */ }
    if (a > 0.4) {
      const c2 = snap(out);
      ox.globalAlpha = a * 0.18;
      ox.drawImage(c2, Math.round((R() - 0.5) * a * w * 0.08), Math.round((R() - 0.5) * a * h * 0.05));
      ox.globalAlpha = 1;
    }
  }
  if (p.damage < 0) { /* repair: soften back toward pristine */
    const a = Math.abs(p.damage) / 100;
    ox.globalAlpha = a * 0.85;
