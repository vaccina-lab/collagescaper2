...
export function renderGlitch(src: GlitchSource, w: number, h: number, p: GlitchParams): { canvas: HTMLCanvasElement; pipe: Pipeline } {
  ...
  /* DAMAGE is the MASTER channel — it was previously dead (never read).
     positive = more chaos: scales every other channel UP and adds its own
       base corruption, so the slider always does something visible.
     negative = repair: scales channels DOWN, then blends back toward the
       pristine original (soften / undo). */
  const dmg = p.damage / 100; /* -1..1 */
  const gain = dmg >= 0 ? 1 + dmg * 1.6 : Math.max(0, 1 + dmg * 0.85);
  const amt = (v: number) => v * gain;
  const inherent = Math.max(0, dmg); /* DAMAGE's own contribution */

  /* inherent damage: even with every other channel at 0, DAMAGE corrupts.
     A little row-slice + rgb-split + static, all seeded & proportional. */
  if (inherent > 0.02) {
    const cuts = Math.round(inherent * 10);
    for (let i = 0; i < cuts; i++) {
      const sy = Math.floor(R() * h); const sh = 2 + Math.floor(R() * (6 + inherent * 26));
      const dx = Math.floor((R() - 0.5) * inherent * w * 0.3);
      ox.drawImage(base, 0, sy, w, sh, dx, sy, w, sh);
    }
  }

  if (amt(p.slice)) {
    const a = Math.abs(amt(p.slice)) / 100, sign = Math.sign(p.slice) || 1;
  ...
  if (amt(p.wave)) {
    const a = Math.abs(amt(p.wave)) / 100, sign = Math.sign(p.wave) || 1;
  ...
  if (amt(p.echo)) {
    const a = Math.abs(amt(p.echo)) / 100, sign = Math.sign(p.echo) || 1;
  ...
  if (amt(p.mirror)) {
    const a = Math.abs(amt(p.mirror)) / 100;
    const c2 = snap(out); ox.clearRect(0, 0, w, h);
  ...
  try {
    ...
    /* inherent rgb split + grain ride in the pixel pass too */
    const inhRgb = Math.round(inherent * w * 0.02);
    if (amt(p.rgb) || inhRgb > 0) {
      const a = Math.abs(amt(p.rgb)) / 100, sign = Math.sign(p.rgb) || 1;
      const shift = Math.round(a * w * 0.045) * sign + inhRgb;
      for (let y = 0; y < h; y++) {
    ...
    const grainAmt = Math.max(amt(p.grain), inherent * 40);
    if (grainAmt > 0) { const a = Math.min(100, grainAmt) / 100; for (let i = 0; i < d.length; i += 4) if (R() < a * 0.28) { const v = (R() - 0.5) * 2 * 255 * a; d[i] += v; d[i + 1] += v; d[i + 2] += v; } }
    if (amt(p.crush)) {
      const a = Math.abs(amt(p.crush)) / 100, sign = Math.sign(p.crush) || 1;
    ...
    if (amt(p.chem)) solarPass(d, Math.abs(amt(p.chem)) / 100, Math.sign(p.chem) || 1);
    if (amt(p.hue)) huePass(d, (amt(p.hue) / 100) * 180);
    if (amt(p.drain)) satPass(d, amt(p.drain) / 100);
    if (amt(p.mosaic)) { const a = Math.abs(amt(p.mosaic)) / 100; const k = Math.max(2, Math.round(2 + a * 30)); for (let y = 0; y < h; y += k) for (let xx = 0; xx < w; xx += k) { const i = (y * w + xx) * 4; const r = d[i], g = d[i + 1], b = d[i + 2]; for (let yy = 0; yy < k && y + yy < h; yy++) for (let dx = 0; dx < k && xx + dx < w; dx++) { const j = ((y + yy) * w + xx + dx) * 4; d[j] = r; d[j + 1] = g; d[j + 2] = b; } } }
    ox.putImageData(img, 0, 0);
  } catch { pipe = 'composite'; }
  if (amt(p.scan)) {
    const a = Math.abs(amt(p.scan)) / 100, sign = Math.sign(p.scan) || 1;
  ...
  /* negative damage = repair: blend back toward pristine, proportional */
  if (dmg < 0) {
    const a = Math.min(1, -dmg);
    ox.globalAlpha = a;
    ox.drawImage(base, 0, 0);
    ox.globalAlpha = 1;
  }
